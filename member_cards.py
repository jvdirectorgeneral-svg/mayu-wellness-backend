...
def draw_spaced_text_with_shadow(draw, position, text, font, spacing=1, text_color=(255, 255, 255), shadow_color=(0, 0, 0)):
    x, y = position
    current_x = x
    ...
    <html>
        ...
        <body style="margin:0; font-family:Arial,sans-serif; background:#0f172a; color:white; display:flex; align-items:center; justify-content:center; min-height:100vh;">
            <div style="max-width:520px; width:90%; background:#1e293b; border-radius:24px; padding:32px;">
                ...
                <p><strong>Código:</strong> {card.member_code}</p>
                <p><strong>Válido hasta:</strong> {CARD_VALIDITY_TEXT}</p>
            </div>
        </body>
    </html>
    """
...
def generate_card_image(user_id: int, db: Session = Depends(get_db)):
    ...
    title_width = get_spaced_text_width(draw, title, title_font, spacing=1)

    draw_spaced_text_with_shadow(
        draw,
        ((width - title_width) // 2, 215),
        title,
        title_font,
        spacing=1,
    )

    draw_text_with_shadow(draw, (60, 305), visual["display_name"], name_font)
    draw_text_with_shadow(draw, (60, 355), visual["level_text"], info_font)
    draw_text_with_shadow(draw, (60, 400), f"Código: {visual['member_code']}", info_font)
    draw_text_with_shadow(draw, (60, 440), f"Válido hasta: {CARD_VALIDITY_TEXT}", info_font)
    draw_text_with_shadow(draw, (60, 480), f"Estado: {card.status}", info_font)
...
def sign_manifest(pass_dir: str, certs_dir: str):
    ...
    if not os.path.exists(wwdr_path):
        raise HTTPException(status_code=500, detail="No existe certs/AppleWWDRCAG3.cer")
    ...
    builder = builder.add_signer(certificate, private_key, hashlib.sha256())
