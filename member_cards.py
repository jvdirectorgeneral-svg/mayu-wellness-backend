...
def generate_member_code(member_number: int, level: int):
    return f"MAYU-{level}-{member_number:06d}"
...
def get_card_code(db: Session, user, card=None):
    ...
    if card and card.id:
        return generate_member_code(get_member_card_number(db, card), user.membership_level)
...
def sync_card_display_code(db: Session | None, user, card):
    if not db or not user or not card:
        return card.member_code if card else ""

    if user.role not in ["member", "ambassador"]:
        raise HTTPException(status_code=400, detail="Solo socios o embajadores pueden tener tarjeta")

    correct_code = get_card_code(db, user, card)
    if correct_code and card.member_code != correct_code:
        card.member_code = correct_code
        db.commit()
        db.refresh(card)

    return card.member_code
...
def build_google_wallet_object_body(user, card, issuer_id: str, class_id: str):
    try:
        ...
        try:
            ...
            fresh_card = temp_db.query(models.MemberCard).filter(models.MemberCard.id == card.id).first() or card
            sync_card_display_code(temp_db, fresh_user, fresh_card)
            card.member_code = fresh_card.member_code
        finally:
            temp_db.close()
    except Exception:
        pass
