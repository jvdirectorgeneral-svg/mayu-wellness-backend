...
def order_to_dict(order: models.MarketplaceOrder):
    ...
    billing_requested = bool(
        getattr(order, "billing_identification", None)
        or getattr(order, "billing_name", None)
        or getattr(order, "billing_email", None)
        or getattr(order, "billing_phone", None)
        or getattr(order, "billing_address", None)
    )
    affiliation_summary = {
        "purchase_type": (
            "direct"
        ...
        "pharmacy_loyalty": {
            ...
            "description": (
                "Acumula puntos cuando Farmacia confirma el pago."
                if pharmacy_code
                else "No vinculada."
            ),
        },
        "doctor_prescriber": {
            ...
            "commission_amount": (
                round(total * doctor_percent / 100, 2) if doctor_code else 0
            ),
            "description": (
        ...
        "billing": {
            "requested": billing_requested,
            "description": (
                "Cliente solicitó factura; revisar los datos completos."
                if billing_requested
                else "Compra sin datos adicionales de facturación."
            ),
        },
    }
    return {
